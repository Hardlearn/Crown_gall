library(dplyr)
data = read.csv("GOslim_age.csv",row.names = 1)

propotion.UCMC <- function(data){
     data_MC = data[data$age=="MC",]
     GO_MC = as.data.frame(table(data_MC$GOSlim))
     colnames(GO_MC)[1] = "GOslim"
     colnames(GO_MC)[2] = "countMC"
     
     data_UC = data[data$age=="UC",]
     GO_UC = as.data.frame(table(data_UC$GOSlim))
     colnames(GO_UC)[1] = "GOslim"
     colnames(GO_UC)[2] = "countUC"
     
     df = merge(GO_MC,GO_UC,by="GOslim",all = TRUE)
...
random <- function(data){
     GO_ID = data$GOSlim
     data$GOSlim = sample(x=GO_ID,size=201754,replace = F)
     return(data)
}
...
# initial
countvector.MC = as.data.frame(integer(95))
colnames(countvector.MC)[1] = "countMC"
countvector.UC = as.data.frame(integer(95))
...
Permutation <- function(data,n){
     ...
     repeat{
          data1 = random(data)
          df1 <- propotion.UCMC(data1)
          loc = match(df$GOslim,df1$GOslim)
          df1 = df1[c(loc),]
