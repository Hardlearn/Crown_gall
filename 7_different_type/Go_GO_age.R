...
Go_result = read.csv("Go_mature_clusterprofiler_result.csv")
Go_gomap_BP = read.table("Go_gomap_BP.txt")
Go_gene_age = read.table("../2_GeneCount/Go_hog_age.txt")
colnames(Go_gomap_BP)[1] = "ID"
colnames(Go_gene_age)[1] = "Gene"

result_GOID = Go_result$ID

data = merge(Go_gomap_BP, Go_result[,c(2,3)], by = "ID")
data = merge(data, Go_gene_age, by="Gene")
data = data[,c(1,2,6)]
...
write.csv(df, 'Go_GO_age_permutation.csv')
